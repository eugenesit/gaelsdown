...
Currently, only the PDF version is fully-functional, with partial
functionality of the gitbook. The Word and epub versions are
developmental, have no templates behind them, and are essentially calls
to the appropriate functions in bookdown. You can view outputs for the
different versions of gaelsdown here:
