...
% developed by Bob Burke (BB) and Dean Jin (DJ), and from the Reed
% College thesis template by Sam Noble (SN).

% Template written in March 2020 for requirements updated March 2017
% for use in the package 'eugensit/gaelsdown' by Eugene Sit (ESS),
% an adaptation of 'ismayc/thesisdown' developed by Chester Ismay (CII).

% The pdf document listing the requirements for Queen's University
% theses has been added to this package as a reference.
...
\documentclass[12pt]{queensuthesis}
...
\numberwithin{equation}{subsection}    % equations will be numbered by subsection
...
\captionsetup{labelsep=period, labelfont=bf, width=5in}
% End of CII addition
...
% Commands passed between YAML (index.Rmd) and LaTeX for title and frontmatter
% This just tells LaTeX where to look for the info
% The sections for the file itself are called after \begin{document}
...
% This section removed by ESS. The instructions for each section are instead
% provided by queensuthesis.cls.
%
% Abstract by default is read in from 00-abstract.Rmd
% instead of from the YAML header
...
% Everything below added by CII
  ...
  \prefacesection{Abstract}{The preface pretty much says it all.
...
Second paragraph of abstract starts here.}
  ...
  \prefacesection{Acknowledgements}{I want to thank a few sentient beings. Optional.}
...
% called from bookdown package in R
  ...
  \prefacesection{Nomenclature and Glossary}{Words that are words but for which you need to specify meaning.}
